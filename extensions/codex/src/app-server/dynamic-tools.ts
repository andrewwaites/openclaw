...
function projectCodexDynamicToolName(
  ...
  if (value.length > CODEX_DYNAMIC_TOOL_NAME_MAX_LENGTH) {
    return { violation: `must be ${CODEX_DYNAMIC_TOOL_NAME_MAX_LENGTH} characters or fewer` };
  }
  if (value.trim() !== value) {
    return { violation: "must not include leading or trailing whitespace" };
  }
  if (value === "mcp" || value.startsWith("mcp__")) {
    return { violation: "is reserved for Codex MCP tools" };
  }
  if (!CODEX_DYNAMIC_TOOL_NAME_PATTERN.test(value)) {
    return {
      violation: "may only contain letters, numbers, underscores, and hyphens",
    };
  }
