...
describe("createCodexDynamicToolBridge", () => {
  ...
  it.each([
    ...
    ["leading whitespace", " fuzzplugin_tool", "must not include leading or trailing whitespace"],
    [
      "invalid character",
      "fuzzplugin bad",
      "may only contain letters, numbers, underscores, and hyphens",
    ],
    ["reserved MCP name", "mcp", "is reserved for Codex MCP tools"],
    ["reserved MCP prefix", "mcp__fuzzplugin", "is reserved for Codex MCP tools"],
    ["too long", `fuzzplugin_${"x".repeat(119)}`, "must be 128 characters or fewer"],
  ])(
    "quarantines invalid dynamic tool names instead of inventing callable names: %s",
    (_label, name, violation) => {
      const bridge = createCodexDynamicToolBridge({
        tools: [
          createTool({
            name,
      ...
      expect(bridge.telemetry.quarantinedTools).toEqual([
        {
          tool: "tool[0]",
          violations: [`tool[0].name ${violation}`],
        },
      ]);
    },
  );
